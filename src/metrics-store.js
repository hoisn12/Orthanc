export class MetricsStore {
  ...
  recordApiCall({ model, durationMs, inputTokens, outputTokens, cacheReadTokens, cacheCreateTokens, costUsd, timestamp }) {
    this.apiCalls.push({
  ...
  recordToolExecution({ toolName, durationMs, success, timestamp }) {
  ...
  recordApiError({ model, errorType, statusCode, timestamp }) {
    this.apiErrors.push({
      model, errorType, statusCode,
      timestamp: timestamp || Date.now(),
    });
    this._prune(this.apiErrors);
  }

  getApiLatencyStats(windowMs = 3600000) {
  ...
  getCostTimeline(bucketMs = 60000) {
  ...
  getModelBreakdown(windowMs = 3600000) {
    const cutoff = Date.now() - windowMs;
    const calls = this.apiCalls.filter((c) => c.timestamp >= cutoff);
    const byModel = {};
    for (const c of calls) {
      if (!byModel[c.model]) byModel[c.model] = { calls: 0, totalLatency: 0, totalCost: 0, totalTokens: 0 };
